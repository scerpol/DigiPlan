...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    const service = body.package ?? body.service ?? "Base";
    const message = body.message ?? "";

    // ✅ SOLO EMAIL OBBLIGATORIA
    if (!email) {
    ...
    // ✅ attachment singolo + nome/tipo dal form
    ...
    // ✅ compatibilità: array attachments
    ...
    await sgMail.send({
      ...
      text:
        `Nome: ${safeName || "(non inserito)"}\n` +
        `Email: ${email}\n` +
        `Pacchetto: ${service}\n\n` +
        `Messaggio:\n${safeMessage || "(nessun messaggio)"}\n`,
      attachments,
    });
